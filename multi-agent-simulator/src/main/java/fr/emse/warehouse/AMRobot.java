...
public class AMRobot extends ColorInteractionRobot<ColorSimpleCell> {
    ...
    private int[] positionHistory2;    // Position 2 ticks ago (for oscillation detection)
    ...
    private static final int MAX_WAIT_TICKS = 1;    // Max ticks to wait before trying alternative (faster response)
    private static final int MAX_STUCK_COUNT = 2;   // Max times to be stuck before full replan
    private static final int TIMEOUT_DELIVERY_TICKS = 12; // Ticks stuck/oscillating before forced timeout-delivery
    private int deliveryStuckTicks = 0;              // Running count of ticks stuck while DELIVERING
    private int totalStuckTicks = 0;                 // Total consecutive ticks stuck (for aggressive escape)
    private int noProgressTicks = 0;                 // Ticks without making progress toward target
    ...
    public AMRobot(String name, int field, int[] pos, Color color, int rows, int columns) {
        ...
        this.positionHistory2 = pos.clone();
        this.stuckCounter = 0;
     ...
     * - Wait patiently when blocked (give others time to move)
     * - Only try escapes after waiting several ticks
     * - Robots must reach exact exit cell to deliver
     */
    private void moveAlongPath() {
        ...
        if (isCellFree(nextPos)) {
            ...
            // Cell is blocked - wait patiently first
            waitCounter++;
            
            // After waiting, try to find an alternative
            if (waitCounter >= 3) {  // Wait 3 ticks before trying alternatives
                // First try: simple recalculation (path may have opened up)
                if (stuckCounter >= 4) {
                    recalculatePath();
                    stuckCounter = 0;
                }
            ...
            // If stuck for longer, try perpendicular escape
            if (totalStuckTicks >= 8) {
                boolean moved = tryPerpendicularEscape(currentPos, nextPos);
                if (moved) {
                    totalStuckTicks = 0;
                }
            }
            
            // Last resort: after many ticks, try random escape (less frequent)
            if (totalStuckTicks >= 15) {
    ...
    /**
     * Try to move perpendicular to the blocked direction to get out of the way.
     */
    private boolean tryPerpendicularEscape(int[] currentPos, int[] blockedPos) {
        ...
        int[][] perpDirs;
        if (dx != 0) {
            // Blocked vertically, try moving horizontally
            perpDirs = new int[][]{{0, -1}, {0, 1}};
        } else {
            // Blocked horizontally, try moving vertically
    ...
    /**
     * Make a random move to break deadlock symmetry.
     */
    ...
    private void onPathCompleted() {
        ...
        switch (state) {
            ...
            case DELIVERING:
                // Path completed - let the simulator check if we're at exit
                // The simulator will handle delivery detection and removal
                // If not at exit yet, recalculate path
                if (targetPosition != null) {
                    int dx = currentPos[0] - targetPosition[0];
                    int dy = currentPos[1] - targetPosition[1];
                    // If close to target (within 2x2 exit area), we're done
                    if (dx >= 0 && dx < 2 && dy >= 0 && dy < 2) {
                        state = State.DELIVERED;
                        palletsDelivered++;
                    } else {
                        // Not there yet - recalculate
    ...
    private boolean isCellFree(int[] pos) {
        ...
        // Check robot position tracker (works even outside perception range)
        if (warehouseEnv != null && warehouseEnv.isOccupiedByRobot(pos, getId())) {
            return false;  // Another robot is at this position
        }
        
        // Also check perception grid for other robots and obstacles
        if (grid != null) {
